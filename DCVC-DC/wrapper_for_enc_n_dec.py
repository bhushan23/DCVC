    ...
    with torch.no_grad():
        for frame_idx in range(frame_num):
            ...
            if frame_idx % gop_size == 0:
                if args['encode_only']:
                    result = i_frame_net.encode_only(x_padded, args['q_in_ckpt'],
                                                    args['i_frame_q_index'], bin_path,
                                                    pic_height=pic_height, pic_width=pic_width)
                elif args['decode_only']:
                    result = i_frame_net.decode_only(bin_path)
                else:
                ...
                if args['encode_only']:
                    result = p_frame_net.encode_only(x_padded, dpb, args['q_in_ckpt'],
                                                    args['i_frame_q_index'], bin_path,
                                                    pic_height=pic_height, pic_width=pic_width,
                                                    frame_idx=frame_idx % 4)
                elif args['decode_only']:
                    result = p_frame_net.decode_only(device, dpb, bin_path)
                else:
                    result = p_frame_net.encode_decode_test(x_padded, dpb, args['q_in_ckpt'],
                                                    args['i_frame_q_index'], bin_path,
                                                    pic_height=pic_height, pic_width=pic_width,
                                                    frame_idx=frame_idx % 4)
                dpb = result["dpb"]
...
def run_encoder(p_frame_net, i_frame_net, args):
    frame_num = args['frame_num']
    gop_size = args['gop_size']
    verbose = args['verbose'] if 'verbose' in args else 0
    device = next(i_frame_net.parameters()).device
    ...
    avg_bpp = sum(bits) / len(bits) / pic_width / pic_height
    folder_name = f"{args['rate_idx']}_{avg_bpp:.4f}"
    file1 = open(os.path.join(args['recon_path'],'bpp.txt'),"w")
    file1.write(folder_name)
    file1.close()

    test_time = time.time() - start_time

    log_result = generate_log_json_simple(frame_num, frame_pixel_num, test_time,
                                       frame_types, bits)
    return log_result

def run_decoder(p_frame_net, i_frame_net, args):
    frame_num = args['frame_num']
    gop_size = args['gop_size']
    device = next(i_frame_net.parameters()).device
    ...
    with torch.no_grad():
        while os.path.exists(bin_path):
            ...
            recon_frame = recon_frame.clamp_(0, 1)
            if ((pic_height is None) and (pic_width is None)) or ((result["pic_height"] != pic_height) or (result["pic_width"] != pic_width)):
                pic_height = result["pic_height"]
                pic_width = result["pic_width"]
                padding_l, padding_r, padding_t, padding_b = get_padding_size(pic_height, pic_width, 16)
            x_hat = F.pad(recon_frame, (-padding_l, -padding_r, -padding_t, -padding_b))
            frame_pixel_num = x_hat.shape[2] * x_hat.shape[3]
...
def encode_one(args):
    ...
    if args['encode_only']:
        result = run_encoder(p_frame_net, i_frame_net, args)
    elif args['decode_only']:
        result = run_decoder(p_frame_net, i_frame_net, args)
    else:
